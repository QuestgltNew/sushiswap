...
// ApesMakerKashi is MasterChef's left hand and kinda a wizard. He can cook up Apes from pretty much anything!
// This contract handles "serving up" rewards for xApes holders by trading tokens collected from Kashi fees for Apes.
contract ApesMakerKashi is Ownable {
    ...
    function _convertStep(address token0, uint256 amount0) private returns (uint256 apesOut) {
        if (token0 == apes) {
            IERC20(token0).safeTransfer(bar, amount0);
            apesOut = amount0;
        } else if (token0 == weth) {
            apesOut = _swap(token0, apes, amount0, bar);
        } else {
            ...
            apesOut = _convertStep(bridge, amountOut);
        }
    }
